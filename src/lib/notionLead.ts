// src/lib/notionLead.ts
//
// Crée un lead dans la base Notion "Deals".
// Best-effort : à appeler depuis un Promise.allSettled, ne doit jamais
// faire échouer la réponse 200 du formulaire.
//
// Le token Notion est passé en argument (lu depuis l'env Cloudflare).
// L'ID de la base n'est pas un secret : constante ci-dessous.
...
export interface NotionLeadInput {
  ...
  telephone?: string;
  entreprise?: string;
  message?: string; // texte déjà composé (besoin, secteur, préférence, message…)
...
export async function createNotionLead(
  ...
  if (input.entreprise) {
    properties['Nom entreprise'] = {
      rich_text: [{ text: { content: input.entreprise.slice(0, 2000) } }],
    };
  }
  if (input.message) {
    properties.Message = {
  ...
  if (typeof input.montant === 'number') {
    properties['Montant estimé'] = { number: input.montant };
  }
